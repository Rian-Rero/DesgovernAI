...
DEFAULT_DEST = "/home/alunos/Desktop/veiculo_real"
...
COLORS = {"verde": "#00cc00", "vermelho": "#cc0000", "roxo": "#8000cc"}
...
def parse_ip_neigh():
    ...
    for line in out.splitlines():
        m = re.search(
            r"(\d+\.\d+\.\d+\.\d+)\s+.*lladdr\s+([0-9a-f:]{17})", line.lower()
        )
        if m:
...
class RsyncGUI(tk.Tk):
    def __init__(self):
        ...
        self.after(
            200, lambda: threading.Thread(target=self.refresh_ips, daemon=True).start()
        )
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, image=self.ufmg_logo).pack(side="right")
    ...
    def _build_tab_home(self, parent):
        ...
        image_path = os.path.join(BASE_DIR, "assets", "wallpaper_fva.png")
    ...
    def _build_tab_files(self, parent):
        ...
        ttk.Label(top, text="Dispositivos detectados (cor quando encontrados):").pack(
            anchor="w"
        )
        ...
        for i, (name, mac) in enumerate(MACS_CARS.items()):
            ...
            cb = ttk.Checkbutton(
                self.devices_frame,
                text=f"{CAR_ICON}{name.upper()} — {mac}",
                variable=var,
        ...
        ttk.Button(
            ...
            command=lambda: threading.Thread(
                target=self.refresh_ips, daemon=True
            ).start(),
        ).pack(side="left", padx=4)
        ttk.Button(btns, text="Selecionar arquivos…", command=self.select_files).pack(
            side="left", padx=4
        )
        ttk.Button(btns, text="Adicionar pasta…", command=self.add_directory).pack(
            side="left", padx=4
        )
        ttk.Button(
            btns, text="Remover selecionado(s)", command=self.remove_selected
        ).pack(side="left", padx=4)
        ttk.Button(btns, text="Limpar seleção", command=self.clear_files).pack(
            side="left", padx=4
        )

        # Lista de arquivos/pastas
        ttk.Label(parent, text="Arquivos/Pastas selecionados:").pack(
            anchor="w", padx=10
        )
        ...
        ttk.Label(
            dest_frame,
            text="Destino na Raspberry (também será o diretório de trabalho dos comandos):",
        ).pack(side="left")
        ...
        ttk.Button(
            send, text="Enviar para selecionados", command=self.send_to_selected
        ).pack(side="left", padx=4)
        ttk.Button(
            send, text="Enviar para todos (com IP)", command=self.send_to_all
        ).pack(side="left", padx=4)
        ...
        for n, c in COLORS.items():
            style.configure(
                f"{n}.TCheckbutton", foreground=c, font=("Arial", 10, "bold")
            )

    # ----------------------------
    def _build_tab_cmds(self, parent):
        ttk.Label(
            parent, text="Executar comandos remotos nas Raspberries selecionadas"
        ).pack(anchor="w", padx=10, pady=(10, 4))

        hint = ttk.Label(
            parent,
            text="Obs.: os comandos serão executados dentro de: (aba Enviar Arquivos) → campo 'Destino na Raspberry'",
            foreground="#888",
        )
        ...
        self.cmd_text.insert("end", 'pkill -f "python3.*main.py"\n' "python3 main.py\n")
        self.cmd_text.pack(fill="x", pady=4)

        ttk.Button(
            parent, text="Executar nos selecionados", command=self.run_cmds_on_selected
        ).pack(pady=6)
        ...
        ttk.Label(plot_select, text="Gráfico:").pack(side="left", padx=(0, 5))
        ...
        self.plot_combo.bind("<<ComboboxSelected>>", lambda event: self.update_plot())
        ...
        ttk.Label(terminal_frame, text="Terminal:").pack(anchor="w")

        self.cmd_log = scrolledtext.ScrolledText(terminal_frame)
    ...
    def _build_tab_data(self, parent):
        ...
        ttk.Label(
            parent, text="Coletar dados dos experimentos armazenados nas Raspberries"
        ...
        ttk.Label(local_frame, text="Destino no computador:").pack(side="left")

        self.data_dest_entry = ttk.Entry(local_frame)
        self.data_dest_entry.insert(0, os.path.join(os.getcwd(), "experimentos"))
        self.data_dest_entry.pack(side="left", fill="x", expand=True, padx=8)

        ttk.Button(
            local_frame, text="Selecionar...", command=self.select_data_destination
        ).pack(side="left")

        # botao de coleta
        ttk.Button(
            parent, text="📥 Coletar dados dos selecionados", command=self.collect_data
        ).pack(anchor="w", padx=10, pady=6)

        # log
        ttk.Label(parent, text="Transferências:").pack(
            anchor="w", padx=10, pady=(10, 2)
        )

        self.data_log = scrolledtext.ScrolledText(parent, height=16)
        self.data_log.pack(fill="both", expand=True, padx=10, pady=(0, 10))
    ...
    def get_selected_devices(self):
        return [
            (n, i["ip"]) for n, i in self.devices.items() if i["var"].get() and i["ip"]
        ]
    ...
    def send_to_selected(self):
        ...
        if not targets:
            messagebox.showinfo(
                "Nenhum alvo", "Selecione pelo menos uma Raspberry com IP."
            )
            return
        threading.Thread(
            target=self._run_rsync_for_targets, args=(targets,), daemon=True
        ).start()
    ...
    def _run_rsync_for_targets(self, targets):
        ...
        for name, ip in targets:
            ...
            if password and has_sshpass:
                ...
                self.log_write(
                    "⚠️ Senha informada mas 'sshpass' não está instalado. Prosseguindo sem sshpass (pode pedir senha)."
                )
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    self.log_write(
                        f"⚠️ Erro (rc={proc.returncode}) em {name.upper()} ({ip})"
                    )
    ...
    def run_cmds_on_selected(self):
        ...
        if not targets:
            messagebox.showinfo(
                "Nenhum alvo", "Selecione ao menos uma Raspberry com IP."
            )
            return
        cmds = [
            c.strip() for c in self.cmd_text.get("1.0", "end").splitlines() if c.strip()
        ]
        if not cmds:
        ...
        threading.Thread(
            target=self._run_remote_cmds, args=(targets, cmds), daemon=True
        ).start()
    ...
    def _run_remote_cmds(self, targets, cmds):
        ...
        for name, ip in targets:
            self.cmdlog_write("\n" + "=" * 60)
            self.cmdlog_write(
                f"💻 Executando carro {name.upper()} ({ip}) - workdir: {remote_workdir}"
            )
            for raw_cmd in cmds:
                ...
                try:
                    ...
                    if proc.returncode != 0:
                        self.cmdlog_write(
                            f"⚠️ Retorno {proc.returncode} para comando: {raw_cmd}"
                        )
                except Exception as e:
    ...
    def collect_data(self):
        ...
        if not local_base:
            messagebox.showinfo(
                "Destino inválido", "Selecione uma pasta para salvar os dados."
            )
            return
        ...
        threading.Thread(
            target=self._collect_data, args=(targets, local_base), daemon=True
        ).start()
    ...
    def _collect_data(self, targets, local_base):
        ...
        for name, ip in targets:
            ...
            self.datalog_write(f"📥 Coletando dados de {name.upper()} ({ip})...")
            ...
            if password and has_sshpass:
                ...
                cmd = ["rsync", "-avz", f"{user}@{ip}:{remote_logs}", local_dest + "/"]
            ...
            try:
                ...
                if proc.returncode == 0:
                    self.datalog_write(f"✅ Dados de {name.upper()} coletados.")
                else:
                    self.datalog_write(
                ...
                self.datalog_write(f"❌ Erro em {name.upper()}: {e}")
    ...
    def update_plot(self):
        ...
        for name, data in self.telemetry.items():
            ...
            if plot_type == "Velocidade":
                ...
                self.ax.plot(data["t"], data["v"], label=f"{name.upper()} - v")

                self.ax.plot(
                    data["t"], data["vref"], "--", label=f"{name.upper()} - vref"
                )

            elif plot_type == "Aceleração / Controle":

                self.ax.plot(data["t"], data["a"], label=f"{name.upper()} - a")

                self.ax.plot(data["t"], data["u"], "--", label=f"{name.upper()} - u")

            elif plot_type == "Velocidade angular":

                self.ax.plot(data["t"], data["w"], label=f"{name.upper()} - w")

            elif plot_type == "Orientação":

                self.ax.plot(data["t"], data["th"], label=f"{name.upper()} - θ")

            elif plot_type == "Trajetória XY":

                self.ax.plot(data["x"], data["y"], label=name.upper())
